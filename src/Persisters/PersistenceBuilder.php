...
final class PersistenceBuilder
{
    ...
    public function prepareInsertData($document): array
    {
        ...
        foreach ($class->fieldMappings as $mapping) {
            $this->applyInsertValue($insertData, $mapping, $this->resolveInsertValue($changeSet, $mapping['fieldName']));
        }
     ...
     */
    private function applyInsertValue(array &$insertData, array $mapping, mixed $new): void
    {
        if ($new === null) {
            if ($mapping['nullable']) {
    ...
    public function prepareUpdateData($document): array
    {
        ...
        foreach ($changeSet->getFieldNames() as $fieldName) {
            ...
            // Scalar fields
            if (! isset($mapping['association'])) {
                $this->applyScalarFieldUpdate($updateData, $mapping, $changeSet->getOldValue($fieldName), $new);
     ...
     * @phpstan-param FieldMapping $mapping
     */
    private function applyScalarFieldUpdate(array &$updateData, array $mapping, mixed $old, mixed $new): void
    {
    ...
    public function prepareUpsertData($document): array
    {
        ...
        foreach ($changeSet->getFieldNames() as $fieldName) {
            ...
            // Scalar fields
            if (! isset($mapping['association'])) {
                $this->applyScalarFieldUpsert($updateData, $mapping, $changeSet->getOldValue($fieldName), $new);
     ...
     * @phpstan-param FieldMapping $mapping
     */
    private function applyScalarFieldUpsert(array &$updateData, array $mapping, mixed $old, mixed $new): void
    {
